...
const DataManager = (() => {
  ...
  async function fetchReliefWebDisasters() {
    try {
          ...
          if (matchedCountry) {
            ...
            if (!duplicate) {
              ...
              const newDisaster = createEvent({
                ...
                businessImpact: "자연재해 발생으로 인한 현지 공장 조업 중단 위험 및 리테일 유통 물류 배송 지연 가능성이 우려됩니다.",
                recommendation: "해당 지역 비상 대책반을 가동하여 안전 상태 점검 및 물류 우회 루트 확보 방안을 마련하십시오.",
                link: urlLink
              });
              
              if (!state.events.find(e => e.title === newDisaster.title)) {
      ...
      console.warn("Could not fetch disasters from ReliefWeb API:", e);
    }
  }

  // 4. Fetch Live News via Backend Proxy
